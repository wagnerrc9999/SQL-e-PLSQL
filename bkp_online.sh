######################################################################################
# Esse script tem por objetivo realizar o backup online do banco de dados           #
...
# mkdir -p /home/oracle/scripts/backup/<ORACLE_SID>/ONLINE                          #
#                                                                                    #
# 2- Copie os scripts bkp_online.sql e bkp_offline.sh para o seguinte local:        #
# cd /home/oracle/scripts/backup/                                                    #
#                                                                                    #
# 3- Mude a permissão do arquivo bkp_online.sh                                      #
# chmod 733 bkp_online.sh                                                           #
#                                                                                    #
...
if [ -z "$1" ]; then
   ...
   echo
   echo 'Sintaxe: bkp_online.sh <SID>'
   echo 'Onde: <SID> é o nome do banco de dados'
   echo
   echo 'Exemplo: bkp_online.sh orcl'
   exit 0
fi
...
if [ "${result}" -ne "0" ] ; then
   ...
   export BKP_DIR=/home/oracle/scripts/backup/$ORACLE_SID/ONLINE
   ...
   conn /as sysdba
   -- Executa o script bkp_online.sql para preparar os comandos de copy
   @/home/oracle/scripts/backup/bkp_online.sql
EOF
   ...
   echo ''
   ls -latr $BKP_DIR
   rm $BIN_DIR/bkp_online_list.sql
fi
